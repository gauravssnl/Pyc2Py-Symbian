#-*- encoding:utf-8 -*-
'''name:powlite_fm_en
# English Translation by gauravssnl
File management module, supports touch screen
...
class manager:
    ...
    def main(s, __Listing=[]):
        if s.path:
            ...
            s.A.app.title=cn('File management')
        seat = s.A.selection_list(__Listing, 1)
        if seat <> None:
            if seat == 0:
                if __Listing[0] == u'<<<':
                    s.path = s.path[:-len(s.path.split('\\')[-2])-1]
                elif __Listing[0] == cn('Recently opened'):
                    s.A.app.title=cn('Recently opened')
                    __Listing = []
                    ...
                    if s.find == 'file':
                        ...
                        if __Listing:
                            for i in __Listing:
                                ...
                                __list += [cn('%s lie in %s'%(path[1], path[0]))]
                            del path
                ...
                if not s.path:
                    ...
                    if s.markmod:
                        ...
                        if __list:
                            ...
                            s.A.note(cn('No tagged catalog!'),'error')
                            s.path = s.path[:-len(s.path.split('\\')[-2])-1]
                            del __list
                    else:
                        s.path = cn(s.path)
    ...
    def scan(s):
        if s.path and not s.O.path.isdir(s.path):
            s.path = None
        if s.path:
            s.A.app.title = cn('please wait……')            
            __list1, __list2 = [u'<<<'], []
            if s.find == 'dir':
                if s.markmod:__list1 += [cn('☆mark the current directory☆')]
                else:__list1 += [cn('☆Select the current directory☆')]
            ...
            if s.CF:
                ...
                __list.sort()
                __Listing = [cn('Recently opened')]+__list
                del __list
            else:__Listing = [cn('Recently opened')]+s.E.drive_list()
        s.main(__Listing)
...
if (__name__ == '__main__'):
    filemain=manager(filepath='d:\\file.txt') # Initialize, the following function is based on this class
    #manager([filepath=None])
    #filepath：Recently open the record path, no no save
    ...
    #ext：Filter the file suffix, multiple with, such as separated ext=['.py', '.pyc']
    #find：Choose a type，find='file'Find the file，find='dir'Find the catalog
    #markmod：markmod=True When the mark mode
    #back：back=True Press Cancel to return to the parent directory, otherwise exit